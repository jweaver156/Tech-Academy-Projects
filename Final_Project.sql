...
;
 ...
 /* 4. For each book that is loaned out from the "Sharpstown" branch and whose DueDate is today,
  retrieve the book title, the borrower's name, and the borrower's address.*/
	...
	CREATE PROC GetTitleNameAndAddress
	AS BEGIN
	...
	WHERE BOOK_LOANS.DateDue = CONVERT (date, GETDATE()) and LIBRARY_BRANCH. BranchName='Sharpstown' 
	END;

	EXEC  GetTitleNameAndAddress;
 ...
 /* 5. For each library branch, retrieve the branch name and the total number of books loaned out from that branch.*/
	...
	FROM BOOK_LOANS INNER JOIN  LIBRARY_BRANCH ON BOOK_LOANS.BranchId = LIBRARY_BRANCH.BranchId
	GROUP BY BranchName 
	END;
 ...
 /* 7. For each book authored (or co-authored) by "Stephen King", retrieve the title and the number of copies
  owned by the library branch whose name is "Central".*/
	...
	CREATE PROCEDURE TitleWithNumberOfCopies
	AS BEGIN
